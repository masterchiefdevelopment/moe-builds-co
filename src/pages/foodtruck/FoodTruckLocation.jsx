    ...
    <div style={{ minHeight: '100vh', padding: '80px 20px 60px', maxWidth: '1100px', margin: '0 auto' }}>
      ...
      <div style={{ marginBottom: '48px', textAlign: 'center' }}>
        ...
        <p style={{ color: '#555', fontSize: '14px', maxWidth: '440px', margin: '0 auto', lineHeight: 1.6 }}>
          We move around San Antonio daily. Check today's stop before you roll out.
        </p>
      </div>
      ...
      <div style={{ background: 'linear-gradient(135deg, #1a1008, #1a1a1a)', border: '1px solid rgba(255,107,43,0.3)', borderRadius: '12px', padding: 'clamp(24px, 4vw, 40px)', marginBottom: '40px' }}>
        <div style={{ marginBottom: '8px', color: '#FF6B2B', fontFamily: "'Barlow Condensed', sans-serif", fontSize: '11px', letterSpacing: '3px', textTransform: 'uppercase' }}>
          Live Now · {TODAY}
        </div>
        <div style={{ fontFamily: "'Bebas Neue', sans-serif", fontSize: 'clamp(28px, 5vw, 48px)', letterSpacing: '3px', lineHeight: 1, marginBottom: '10px' }}>
          {TODAY_SPOT.location}
        </div>
        <div style={{ color: '#888', fontSize: '14px', marginBottom: '4px' }}>📍 {TODAY_SPOT.address}</div>
        <div style={{ color: '#555', fontSize: '13px', marginBottom: '24px' }}>⏰ {TODAY_SPOT.hours}</div>

        <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap', marginBottom: '24px' }}>
          
            href={"https://maps.google.com/?q=" + encodeURIComponent(TODAY_SPOT.address)}
            target="_blank"
            rel="noreferrer"
            style={{ background: '#FF6B2B', color: '#0A0A0A', borderRadius: '6px', fontFamily: "'Barlow Condensed', sans-serif", fontSize: '13px', fontWeight: 700, letterSpacing: '2px', textTransform: 'uppercase', textDecoration: 'none', padding: '12px 20px' }}
          >
            Get Directions
          </a>
          <Link
            to="/foodtruck/order"
            style={{ background: 'transparent', color: '#FF6B2B', border: '1px solid rgba(255,107,43,0.3)', borderRadius: '6px', fontFamily: "'Barlow Condensed', sans-serif", fontSize: '13px', letterSpacing: '2px', textTransform: 'uppercase', textDecoration: 'none', padding: '12px 20px' }}
          >
            Order Ahead
          </Link>
        </div>
        ...
        <div style={{ width: '100%', height: '200px', borderRadius: '8px', border: '1px solid #2a2a2a', background: '#111', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '12px' }}>
          ...
          <div style={{ fontSize: '12px', color: '#555' }}>{TODAY_SPOT.address}</div>
            ...
            target="_blank"
            rel="noreferrer"
            style={{ background: '#FF6B2B', color: '#0A0A0A', borderRadius: '6px', fontFamily: "'Barlow Condensed', sans-serif", fontSize: '12px', fontWeight: 700, letterSpacing: '2px', textTransform: 'uppercase', textDecoration: 'none', padding: '10px 22px' }}
          >
            Open in Google Maps
          </a>
        </div>
            ...
            return (
              <div key={stop.day} style={{ background: isToday ? 'rgba(255,107,43,0.06)' : '#161616', border: '1px solid ' + (isToday ? 'rgba(255,107,43,0.35)' : '#1f1f1f'), borderRadius: '8px', padding: '16px 20px', display: 'flex', alignItems: 'center', gap: '16px', flexWrap: 'wrap' }}>
                <div style={{ fontFamily: "'Bebas Neue', sans-serif", fontSize: '18px', letterSpacing: '2px', color: isToday ? '#FF6B2B' : '#555', minWidth: '100px' }}>
                  {stop.day} {isToday && <span style={{ background: '#FF6B2B', color: '#0A0A0A', borderRadius: '100px', padding: '1px 8px', fontSize: '9px', fontFamily: "'Barlow Condensed', sans-serif", letterSpacing: '1.5px', textTransform: 'uppercase' }}>Today</span>}
                </div>
                ...
                <div style={{ fontFamily: "'Barlow Condensed', sans-serif", fontSize: '13px', color: isToday ? '#FF6B2B' : '#555', whiteSpace: 'nowrap' }}>⏰ {stop.hours}</div>
                
                  href={"https://maps.google.com/?q=" + encodeURIComponent(stop.address)}
                  target="_blank"
                  rel="noreferrer"
                  style={{ fontFamily: "'Barlow Condensed', sans-serif", fontSize: '11px', letterSpacing: '1.5px', textTransform: 'uppercase', textDecoration: 'none', color: '#444', border: '1px solid #2a2a2a', borderRadius: '4px', padding: '6px 12px', whiteSpace: 'nowrap' }}
                >
                  Directions
                </a>
              </div>
      ...
      <div style={{ background: '#161616', border: '1px solid #1f1f1f', borderRadius: '10px', padding: '28px 24px', display: 'flex', flexWrap: 'wrap', alignItems: 'center', justifyContent: 'space-between', gap: '20px' }}>
        <div>
          ...
          <p style={{ fontSize: '13px', color: '#555', lineHeight: 1.5 }}>Follow us on Instagram for real-time updates, daily specials, and pop-up events.</p>
        </div>
        <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap' }}>
          <a href="#" style={{ background: '#FF6B2B', color: '#0A0A0A', borderRadius: '6px', fontFamily: "'Barlow Condensed', sans-serif", fontSize: '13px', fontWeight: 700, letterSpacing: '2px', textTransform: 'uppercase', textDecoration: 'none', padding: '12px 22px' }}>
            📸 @moesfoodtruck
          </a>
          <Link to="/foodtruck/order" style={{ background: 'transparent', color: '#888', border: '1px solid #2a2a2a', borderRadius: '6px', fontFamily: "'Barlow Condensed', sans-serif", fontSize: '13px', letterSpacing: '2px', textTransform: 'uppercase', textDecoration: 'none', padding: '12px 22px' }}>
            Order Ahead
          </Link>
        </div>
      </div>
