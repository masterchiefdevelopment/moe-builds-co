  ...
  return (
    <div onClick={onClose} style={{
      ...
      <div onClick={e => e.stopPropagation()} style={{
        ...
        <h2 style={{ fontFamily: "'Playfair Display', serif", fontSize: 26, fontWeight: 700, color: '#fff', marginBottom: 4 }}>
          {mode === 'signin' ? 'Welcome Back' : 'Join Jalisco Grill'}
        </h2>
        <p style={{ color: C.textSecondary, fontSize: 13, marginBottom: 22 }}>
          {mode === 'signin' ? 'Sign in to view your rewards and order history.' : 'Earn loyalty rewards on every order.'}
        </p>
        ...
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          {mode === 'signup' && (
            <input className="rg-input" placeholder="Full name" value={form.fullName} onChange={e => set('fullName', e.target.value)} style={{ marginTop: 0 }} />
          )}
          <input className="rg-input" type="email" placeholder="Email" value={form.email} onChange={e => set('email', e.target.value)} style={{ marginTop: 0 }} />
          <input className="rg-input" type="password" placeholder="Password" value={form.password} onChange={e => set('password', e.target.value)} style={{ marginTop: 0 }} />
          <button type="submit" disabled={loading} className="rg-btn-gold" style={{ marginTop: 4 }}>
