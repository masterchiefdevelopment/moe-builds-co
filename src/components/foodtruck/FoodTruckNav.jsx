  ...
  return (
    <>
      <header style={{
        ...
        <a href="#home" onClick={goToSection('home')} style={{ display: 'flex', alignItems: 'center', gap: 10, textDecoration: 'none', cursor: 'pointer' }}>
          <span style={{
            ...
            background: C.accentColor, fontSize: 17,
          }}>🚚</span>
          <span style={{ fontFamily: "'Bebas Neue', sans-serif", fontSize: 22, letterSpacing: 1.5, color: '#fff' }}>
            {C.name}
          </span>
        </a>
        ...
        <nav style={{ display: 'flex', gap: '4px' }} className="ft-nav-desk">
          {LINKS.map(({ label, id }) => (
            <a key={id} href={`#${id}`} onClick={goToSection(id)} className="ft-nav-link" style={{
              ...
              textDecoration: 'none', padding: '8px 14px', color: isActive(id) ? C.accentAmber : '#fff',
              position: 'relative', cursor: 'pointer',
            }}>
              {label}
              <span style={{
                position: 'absolute', left: 14, right: 14, bottom: 2, height: 2, borderRadius: 1,
                background: C.accentAmber, transform: isActive(id) ? 'scaleX(1)' : 'scaleX(0)',
      ...
      <nav ref={drawerRef} style={{
        ...
        {LINKS.map(({ label, id }) => (
          <a key={id} href={`#${id}`} onClick={goToSection(id)} style={{
            ...
            color: isActive(id) ? C.accentAmber : '#fff', padding: '10px 0', borderBottom: `1px solid ${C.borderSubtle}`,
          }}>{label}</a>
        ))}
          ...
          {user ? (
            ...
            <>
              ...
              <a href="#order" onClick={goToSection('order')} style={{
                ...
                textDecoration: 'none', color: '#0d0d0d', background: C.accentColor, padding: '12px 16px', borderRadius: 8, textAlign: 'center', cursor: 'pointer',
              }}>Order Now</a>
            </>
          )}
        </div>
      ...
      <style>{`
        ...
        .ft-nav-link:hover { color: ${C.accentAmber} !important; }
        .ft-order-btn:hover { transform: scale(1.04); filter: brightness(1.1); }
        .ft-ghost-btn:hover { color: #fff !important; border-color: ${C.accentColor} !important; }
